...
void UnifiedParticleTransformerAK4ONNXJetTagsProducer::get_input_sizes(
  ...
  if (use_dynamic_axes_) {
    ...
    n_cpf_ = std::clamp((unsigned int)features.c_pf_features.size(), (unsigned int)1, (unsigned int)UparT::n_cpf_accept);
    n_lt_ = std::clamp((unsigned int)features.lt_features.size(), (unsigned int)1, (unsigned int)UparT::n_lt_accept);
    n_npf_ = std::clamp((unsigned int)features.n_pf_features.size(), (unsigned int)1, (unsigned int)UparT::n_npf_accept);
